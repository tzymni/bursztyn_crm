...
export default {
  ...
  methods: {
    ...
    changeColor: function () {
      ...
      square.style.color = this.switchColor()
    },
    switchColor: function () {
      if (this.cottage_id === null) {
        return "#fff"
      } else {
        let cottage_id = this.cottage_id
        let color_value = this.options.findIndex((x) => x.value === cottage_id)
        return this.options[color_value].color
      }
    },
